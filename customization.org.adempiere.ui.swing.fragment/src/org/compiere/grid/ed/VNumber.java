	...
	public void setValue(Object value)
	{
		...
		{								//JPIERE-3 Modify VNumber#setValue() by Hideaki Hagiwara
			if( m_text != null )
			{
				if( m_mField != null )
				{
					if (m_mField.getFormatPattern() != null)
					{
						DecimalFormat format = new DecimalFormat(m_mField.getFormatPattern() );
						m_format.setMinimumFractionDigits(format.getMinimumFractionDigits());
					}else{
						if( m_displayType==DisplayType.Amount )
						{
							m_format.setMinimumFractionDigits(MCurrency.get(Env.getCtx()
									,Env.getContextAsInt (m_mField.getVO().ctx, m_mField.getVO().WindowNo, m_mField.getVO().TabNo, "C_Currency_ID")).getStdPrecision());
						}else if(m_displayType==DisplayType.CostPrice){
							m_format.setMinimumFractionDigits(MCurrency.get(Env.getCtx()
									,Env.getContextAsInt (m_mField.getVO().ctx, m_mField.getVO().WindowNo, m_mField.getVO().TabNo, "C_Currency_ID")).getCostingPrecision());
						}
					}	//if (m_mField.getFormatPattern() != null)
				}	//if( m_mField != null )
