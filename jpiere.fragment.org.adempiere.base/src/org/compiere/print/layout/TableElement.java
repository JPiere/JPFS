			...
			else
			{
				...
				if( col != dataCol && m_multiLineHeader){
					...
					if( width < headerSizes[dataCol].getWidth())
						width = headerSizes[dataCol].getWidth();
					double height = headerSizes[col].getHeight() + headerSizes[dataCol].getHeight();
...
//		int tempCurY = curY;	//JPIERE-4-3 Modify TableElement#printColumn() by Hideaki Hagiwara
		...
		{
			int header_col = cols[i];
			if (m_columnHeader[header_col].toString().length() > 0)
			{
				aString = new AttributedString(m_columnHeader[header_col].toString());
				aString.addAttribute(TextAttribute.FONT, getFont(HEADER_ROW, header_col));
				aString.addAttribute(TextAttribute.FOREGROUND, getColor(HEADER_ROW, header_col));
				//
				boolean fastDraw = LayoutEngine.s_FASTDRAW;
				if (fastDraw && !isView && !Util.is8Bit(m_columnHeader[header_col].toString()))
					fastDraw = false;
				...
				while (measurer.getPosition() < iter.getEndIndex())//print header
				{
					...
					if (iter.getEndIndex() != measurer.getPosition())
						fastDraw = false;
					Float lineHeight = layout.getAscent() + layout.getDescent() + layout.getLeading();
					if (m_columnMaxHeight[header_col] <= 0//-1 = FirstLineOnly
							|| (usedHeight + lineHeight) <= m_columnMaxHeight[header_col])
					{
						...
						{//Bug - set Font/Color explicitly
							g2D.setFont(getFont(HEADER_ROW, header_col));
							g2D.setColor(getColor(HEADER_ROW, header_col));
	 ...
	 *
	 */
	private Integer[] getPrintHeaderCol( int col ){
		...
		for (int dataCol = 0; dataCol < cols; dataCol++)
		{
			...
			if (m_additionalLines.containsKey(new Integer(dataCol)))
			{
				int col_new = ((Integer)m_additionalLines.get(new Integer(dataCol))).intValue();
				if( col_new == col ){
					list.add( new Integer(dataCol) );
				}
			log.finest("DataColumn=" + dataCol + ", BelowColumn=" + col_new);
			}
		}
