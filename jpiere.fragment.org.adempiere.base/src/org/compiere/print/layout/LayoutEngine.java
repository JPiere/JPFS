						...
						else if (obj instanceof PrintDataElement)
						{
							...
							{				//JPIERE-3 Modify LayoutEngine#layoutTable by Hideaki Hagiwara
								if(pde.getDisplayType()==DisplayType.Amount || pde.getDisplayType()==DisplayType.CostPrice)
								{
									...
									if(C_Currency_ID==0 && m_data != null)
									{
										dataElement = getValueDisplay(format.getLanguage(), getC_Currency_ID(m_data), pde);//use currency of parent Data.
									}else{
										dataElement = getValueDisplay(format.getLanguage(), C_Currency_ID ,pde);
									}
								}else{
